...
//! (hdfs://, s3://, ...) through whatever file system is registered in
//! DuckDB. Holds the database instance, so it stays valid when cached across
//! statements (e.g. inside an attached catalog).
class DuckDBVfsFileSystem : public paimon::FileSystem {
public:
	explicit DuckDBVfsFileSystem(shared_ptr<DatabaseInstance> db);
